...
- `apps/api` — `@vercel/python` builder via `vercel.json`, dependencies pinned in `uv.lock` (see "Lesson learned" below). Env vars: `DATABASE_URL` (transaction pooler, port 6543), `GROQ_API_KEY`, `LIVEBLOCKS_SECRET_KEY`, `FRONTEND_ORIGIN` (the frontend project's URL, so `main.py`'s CORS middleware allows it), `FRONTEND_BASE_URL` (where the Jira OAuth callback sends the browser back to, see ADR-0007), and optionally `JIRA_*` / `JIRA_OAUTH_*` / `TOKEN_ENCRYPTION_KEY` / `AZURE_DEVOPS_*`.
...
## Lesson learned: `@vercel/python` actually reads `uv.lock`, not `requirements.txt`

Adding the `cryptography` dependency for ADR-0007's Jira OAuth work caused a full production outage (every route, including `/health`, returned 500 with `ModuleNotFoundError: No module named 'cryptography'`) even after `requirements.txt` was updated and redeployed. The build logs showed the actual cause: `@vercel/python` installs from **`uv.lock`** when one is present, ignoring `requirements.txt` entirely — the opposite of what a long-standing comment in that file claimed. `requirements.txt` had silently stopped being the real source of truth for the deployed build at some earlier point without anyone noticing, since no dependency had been added since. Fixed by running `uv lock` to regenerate the lockfile with the new dependency; `requirements.txt` is kept in sync for other tooling but is no longer trusted as the deploy-time source of truth. Any future dependency addition must include running `uv lock`, not just editing `requirements.txt`/`pyproject.toml`.
